...
int main() {
	...
	sf::RectangleShape marketBut({ 100.f, 100.f }); // filler
	marketBut.setPosition({ 1200.f, 800.f });
	marketBut.setOrigin({ 50.f, 50.f });
	marketBut.setFillColor(sf::Color::Green);
			...
			if (const sf::Event::MouseButtonPressed* mouseClicked = event->getIf<sf::Event::MouseButtonPressed>()) {
				if (mouseClicked->button == sf::Mouse::Button::Left) {
					...
					if (state == gameState::Menu1 && !playButtonClicked) {
						...
						if (marketBut.getGlobalBounds().contains(mappedPos)) {
