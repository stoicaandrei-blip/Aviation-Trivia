...
bool achievmentNot = true;
int questionStreak = 0;
...
int main() {
	...
	if (!marketTexture.loadFromFile("market.png")) {
		std::cout << "Couldn't load market texture! Make sure you have <<market.png>>\n";
		problem = true;
	}
	...
	std::cout << "\n";
	std::cout << "----------------------------------------------\n\n";
	std::cout << "Enter <<commands>> to view all commands\n\n";
	...
	sf::Sprite marketButton(marketTexture);
	...
	volume.setPosition({ 825.f, 375.f });
	marketButton.setPosition({ 1200.f, 800.f });
	...
	volume.setOrigin({ 9.f, 3.5f });
	marketButton.setOrigin({ 19.f, 5.5f });
	...
	volume.setScale({ 6.f, 6.f });
	marketButton.setScale({8.f, 8.f});
	...
	sf::Sprite SFXvol = volume;
	SFXvol.setPosition({ 1075.f, 425.f });

	sf::Sprite achNot = checkBox;
	achNot.setPosition({ 1150.f, 475.f });
	...
	while(window.isOpen()){
		...
		while (!commands.empty()) {
			...
			if (command == "fileNames") {
				...
				std::cout << "volume.png - 144 x 7 (SpSh 8)\n";
				...
				if (skipIntro) {
					...
					std::cout << "Don't skip intro\n";
				}
				std::cout << "Music volume: " << musicVolume << "\n";
				std::cout << "Sound effects volume: " << SFXVolume << "\n";
				if (achievmentNot) {
					std::cout << "Receive achievment notification\n";
				}
				else {
					std::cout << "Don't receive achievment notification\n";
				}
				std::cout << "\n";
				std::cout << "----------------------------------------------\n\n";
			}
				...
				std::cout << "settings -- shows all settings and their configuration\n";
				...
				if (mouseClicked->button == sf::Mouse::Button::Left) {
					...
					if (state == gameState::Menu1 && !playButtonClicked) {
						...
						if (feedbackButton.getGlobalBounds().contains(mappedPos)) {
							openURL("https://docs.google.com/forms/d/e/1FAIpQLSd_PYRWfn150R5Bi3gj90nlJ-TgKZuzrlvwGKAd-GypAZUxvg/viewform?usp=dialog");
						}
						if (moreGames.getGlobalBounds().contains(mappedPos)) {
							openURL("https://github.com/stoicaandrei-blip");
						}
						if (exitButton.getGlobalBounds().contains(mappedPos)) {
							window.close();
						}
						...
						if (marketButton.getGlobalBounds().contains(mappedPos)) {
							state = gameState::MarketMenu;
						}
					}
						...
						if (volume.getGlobalBounds().contains(mappedPos)) {
							if (musicVolume == 100.f) {
								musicVolume = 0.f;
							}
							else if (musicVolume == 75.f) {
								musicVolume = 100.f;
							}
							else if (musicVolume == 50.f) {
								musicVolume = 75.f;
							}
						...
						if (SFXvol.getGlobalBounds().contains(mappedPos)) {
							if (SFXVolume == 100.f) {
								SFXVolume = 0.f;
							}
							else if (SFXVolume == 75.f) {
								SFXVolume = 100.f;
							}
							else if (SFXVolume == 50.f) {
								SFXVolume = 75.f;
							}
							else if (SFXVolume == 0.f) {
								SFXVolume = 50.f;
							}
							saveStats();
						}
						if (achNot.getGlobalBounds().contains(mappedPos)) {
							if (achievmentNot == true) {
								achievmentNot = false;
							}
							else if (achievmentNot == false) {
								achievmentNot = true;
							}
							saveStats();
						}
					}
					else if (state == gameState::MarketMenu) {
						if (arrow.getGlobalBounds().contains(mappedPos)) {
							state = gameState::MoreMenu;
						}
						if (exitButton.getGlobalBounds().contains(mappedPos)) {
		...
		if (state == gameState::Menu1 && !playButtonClicked) {
			...
			if (marketButton.getGlobalBounds().contains(mappedMousePos)) {
				marketButton.setTextureRect(sf::IntRect({ 38, 0 }, { 38, 11 }));
			}
			else {
				marketButton.setTextureRect(sf::IntRect({ 0, 0 }, { 38, 11 }));
			}
		}
		else if (state == gameState::CreditsMenu) {
			if (exitButton.getGlobalBounds().contains(mappedMousePos)) {
			...
			if (!achievmentNot) {
				if (achNot.getGlobalBounds().contains(mappedMousePos)) {
					achNot.setTextureRect(sf::IntRect({ 9, 0 }, { 9, 9 }));
				}
				else {
					achNot.setTextureRect(sf::IntRect({ 0,0 }, { 9, 9 }));
				}
			}
			else if (achievmentNot) {
				if (achNot.getGlobalBounds().contains(mappedMousePos)) {
					achNot.setTextureRect(sf::IntRect({ 27, 0 }, { 9,9 }));
				}
				else {
					achNot.setTextureRect(sf::IntRect({ 18, 0 }, { 9,9 }));
				}
			}
			...
			if (musicVolume == 100.f) {
				if (volume.getGlobalBounds().contains(mappedMousePos)) {
					volume.setTextureRect(sf::IntRect({ 126, 0 }, { 18, 7 }));
				}
				else {
					volume.setTextureRect(sf::IntRect({ 108, 0 }, { 18, 7 }));
				}
			}
			else if (musicVolume == 75.f) {
				if (volume.getGlobalBounds().contains(mappedMousePos)) {
					volume.setTextureRect(sf::IntRect({ 90, 0 }, { 18, 7 }));
				}
				else {
					volume.setTextureRect(sf::IntRect({ 72, 0 }, { 18, 7 }));
				}
			}
			else if (musicVolume == 50.f) {
				if (volume.getGlobalBounds().contains(mappedMousePos)) {
					volume.setTextureRect(sf::IntRect({ 54, 0 }, { 18, 7 }));
				}
				else {
					volume.setTextureRect(sf::IntRect({ 36, 0 }, { 18, 7 }));
				}
			}
			else if (musicVolume == 0.f) {
				if (volume.getGlobalBounds().contains(mappedMousePos)) {
					volume.setTextureRect(sf::IntRect({ 18, 0 }, { 18, 7 }));
				}
				else {
					volume.setTextureRect(sf::IntRect({ 0, 0 }, { 18, 7 }));
				}
			}
			if (SFXVolume == 100.f) {
				if (SFXvol.getGlobalBounds().contains(mappedMousePos)) {
					SFXvol.setTextureRect(sf::IntRect({ 126, 0 }, { 18, 7 }));
				}
				else {
					SFXvol.setTextureRect(sf::IntRect({ 108, 0 }, { 18, 7 }));
				}
			}
			else if (SFXVolume == 75.f) {
				if (SFXvol.getGlobalBounds().contains(mappedMousePos)) {
					SFXvol.setTextureRect(sf::IntRect({ 90, 0 }, { 18, 7 }));
				}
				else {
					SFXvol.setTextureRect(sf::IntRect({ 72, 0 }, { 18, 7 }));
				}
			}
			else if (SFXVolume == 50.f) {
				if (SFXvol.getGlobalBounds().contains(mappedMousePos)) {
					SFXvol.setTextureRect(sf::IntRect({ 54, 0 }, { 18, 7 }));
				}
				else {
					SFXvol.setTextureRect(sf::IntRect({ 36, 0 }, { 18, 7 }));
				}
			}
			else if (SFXVolume == 0.f) {
				if (SFXvol.getGlobalBounds().contains(mappedMousePos)) {
					SFXvol.setTextureRect(sf::IntRect({ 18, 0 }, { 18, 7 }));
				}
				else {
					SFXvol.setTextureRect(sf::IntRect({ 0, 0 }, { 18, 7 }));
				}
			}
		}
		else if (state == gameState::MarketMenu) {
		...
		if (state == gameState::Intro) {
			...
			if (playButtonClicked) {
				...
				drawText(window, fontTexture, "QUESTION STREAK " + std::to_string(questionStreak), {1300.f, 1000.f}, 6);
			...
			drawText(window, fontTexture, "MUSIC VOLUME", { 400.f, 360.f }, 6);
			window.draw(volume);
			drawText(window, fontTexture, "SOUND EFFECTS VOLUME", { 400.f, 410.f }, 6);
			window.draw(SFXvol);
			drawText(window, fontTexture, "ACHIEVMENT NOTIFICATION", { 400.f, 460.f }, 6);
			window.draw(achNot);
			drawText(window, fontTexture, "ONLY THE FIRST TWO ARE FUNCTIONAL", { 300.f, 600.f }, 8);
		}
		else if (state == gameState::MarketMenu) {
			window.draw(gameBackground);
			window.draw(arrow);
			window.draw(exitButton);
			window.draw(box);
			drawText(window, fontTexture, "MARKET", { 800.f, 100.f }, 8);
			drawText(window, fontTexture, "COMING SOON", { 700.f, 400.f }, 9);
		}
		window.display();
